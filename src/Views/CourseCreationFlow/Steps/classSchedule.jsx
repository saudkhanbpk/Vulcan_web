...
console.log('course_times', userData?.educator?.courses?.pending?.class_schedule)
  ...
  useEffect(() => {
    ...
    if (course_times && typeof course_times === "object") {
      ...
      // Loop through each day in the form data
      for (const day in formData) {
        if (course_times[day]) {
          updatedFormData[day] = {
            // Directly convert timestamps to dayjs objects and convert to UTC
            start: dayjs.unix(course_times[day].start),
            end: dayjs.unix(course_times[day].end),
            checked: true,
          };
  
          // Log the converted time data for debugging
          console.log("timeDataBases", course_times[day]);
        }
      }
  
      // Set the updated form data with converted times
      setFormData(updatedFormData);
    }
  
    // Removed exhaustive-deps warning
